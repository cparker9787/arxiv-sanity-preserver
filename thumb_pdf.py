...
def main():
    ...
    for i,p in enumerate(pdf_files):
        ...
        # (using a roundabout alternative that requires temporary files but succeeds more often)
        ...
        if os.path.isfile(os.path.join(Config.tmp_dir, 'thumb-0.png')):
            for i in range(8):
                ...
                if os.path.isfile(f):
                    cmd = 'mv %s %s' % (f, f2)
                    os.system(cmd)
                    # using mv instead of rm to be safer; needed because if some papers are
                    # shorter than 8 pages, results from previous paper will "leek" over via
